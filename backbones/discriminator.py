...
class Discriminator_large(nn.Module):
  ...
  def forward(self, x, t, x_t):
    ...
    h1 = self.start_conv(input_x)
    h2 = self.conv1(h1,t_embed)    
   
    h3 = self.conv2(h2,t_embed)
   
    h4 = self.conv3(h3,t_embed)
    

    h5 = self.conv4(h4,t_embed)
    h6 = self.conv5(h5,t_embed)
   
    
    out = self.conv6(h6,t_embed)
...
class Discriminator_img_large(nn.Module):
  """A time-dependent discriminator for large images (CelebA, LSUN)."""
  ...
  def __init__(self, nc = 1, ngf = 32, t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
    ...
    # self.att_conv = conv2d(ngf * 8, 1, 1, padding=0)
  ...
  def forward(self, x, t, x_t):
    ...
    # mid_att_map = self.att_conv(F.sigmoid(self.mid_feat))
    # mid_att_map_upsampled = F.interpolate(mid_att_map, size=(256,256), mode='bilinear', align_corners=False)
    ...
    out = out.view(out.shape[0], out.shape[1], -1).sum(2)
    out = self.end_linear(out)
    # mid_att_map = self.att_conv(F.sigmoid(mid_feat))
